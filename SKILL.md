...
|---|---|---|
| Phase 1.1-1.2 内容编写 | `references/SCRIPT-STYLE.md` + `references/OUTLINE-FORMAT.md` + `article.md`（用户原文，如有） | **PDF 源料**：先走 [`references/PDF-INPUT.md`](references/PDF-INPUT.md) 桥接成 `article.md` 再进本表流程 |
...
| **PDF 文件**（论文 / 报告 / 白皮书） | 先走 [`references/PDF-INPUT.md`](references/PDF-INPUT.md) 的桥接：Read 工具逐页提取 → 清洗 → 落盘 `article.md`，**之后**走原生流程 |
...
| [`references/PDF-INPUT.md`](references/PDF-INPUT.md) | Phase 1.1 触发（用户给 PDF 时） | PDF → `article.md` 桥接流程 + 图/公式/扫描件/超长 PDF 边界 |
